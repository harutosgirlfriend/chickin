...
@section('content')
    <div class="space-y-8">
        @if ($produkKurangStok->isNotEmpty())
            <div class="w-full bg-red-600 text-white p-4 rounded shadow mb-6">
                <p class="font-bold mb-2">Peringatan: Stok Produk Kurang!</p>
                <p class="text-sm">
                    @foreach ($produkKurangStok as $produk)
                        {{ $produk->nama_product }} (Stok: {{ $produk->stok }}, Minimal: {{ $produk->minimal_stok }})
                        @if (!$loop->last)
                            ,
                        @endif
                    @endforeach
